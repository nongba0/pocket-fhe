...
int main() {
    std::cout << "=== Pocket-FHE Homomorphic Computation Recovery Test (fhe_core.hpp Unified) ===" << std::endl;
    std::cout << "[Noise Model Notice] LUT/EvalMod are noise-model simulated; timing reflects pure glue." << std::endl;
    ...
    // Evaluate f(m) = ReLU(2m + 5) homomorphically
    ...
    for (int j = 0; j < k; ++j) {
        ...
        for (int i = 0; i < n; ++i) {
            int64_t orig = (i * 2 + j) % 80 - 40;
            m[i] = std::max<int64_t>(0, 2 * orig + 5); // ReLU(2m+5)
            ctsA[j][i] = dist_q(gen);
        }
    ...
    std::cout << "[Homomorphic Computation Result] Exact Recovery: " << exact << " / " << N << " slots" << std::endl;
    if (exact == N) {
        std::cout << "✅ Homomorphic Computation Recovery PASS (f(m) = ReLU(2m+5) 100% PASS)" << std::endl;
    } else {
        std::cerr << "❌ Homomorphic Computation Recovery FAIL" << std::endl;
        return 1;
    }
